...
inline __host__ __device__ int get_backward_window_end(const int index, const int length, const int KERNEL_SIZE, const int NEIGHBORHOOD_SIZE)
{
    return (index >= length - KERNEL_SIZE) ? (length) : (index + (NEIGHBORHOOD_SIZE + 1));
}


inline __host__ __device__ int get_window_start(const int index, const int length, const int KERNEL_SIZE, const int NEIGHBORHOOD_SIZE)
{
    return  max(index - NEIGHBORHOOD_SIZE, 0) + (index + NEIGHBORHOOD_SIZE >= length) * (length - index - NEIGHBORHOOD_SIZE - 1);
}


inline __host__ __device__ int get_pb_start(const int index, const int length, const int KERNEL_SIZE, const int NEIGHBORHOOD_SIZE)
{
...
#define CHECK_SEQUENCE(length, kernel_size) TORCH_CHECK(length >= kernel_size, "Input sequence length must be greater than or equal to kernel size.")
#define CHECK_FEATMAP(height, width, kernel_size) TORCH_CHECK(height >= kernel_size && width >= kernel_size, "Input resolution must be greater than or equal to kernel size.")
